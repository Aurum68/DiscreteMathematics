...
max_matching, matching_edges = graph_ford.find_matching_ford()
...
max_matching = graph_khun.find_matching_khun()
max_matching_set = []
for i in range(len(max_matching)):
    edge = Edge(i, max_matching[i]) if max_matching[i] != -1 else None
    if not edge  is None:
        ...
        # for j in range(len(max_matching_set)):
        #     if max_matching_set[j] == edge:
        #         break
